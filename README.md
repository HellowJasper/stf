...
当前调用地址为 `https://api.deepseek.com/chat/completions`，默认模型为 `deepseek-v4-pro`。主聊天公开分析、下一句建议、玄学档案与实时聊天加深都使用 JSON Output，即让模型按固定 JSON（JavaScript Object Notation，JavaScript 对象表示法）结构返回内容，便于前端稳定渲染。模型失败时自动切换到本地演示引擎，保证现场流程不断。
...
4. DeepSeek 与 1–3 秒等待并行工作；等待结束后，Python 先按本地场景规则流式发送可信的对方反馈，确保首段响应不被模型延迟拖慢。
5. DeepSeek 根据用户原话、最近聊天，以及外挂开启时的沟通偏好、雷点和开心开关，重新生成公开分析摘要与下一句建议；超过 Demo 时限则自动使用已增强的本地结果。

八字外挂不会在场景之间继承：每次进入新场景都恢复关闭。用户手动开启时，会先经过 1–2 秒“沟通命盘校准”，加载完成后才影响下一轮公开分析与建议话术。
...
- 主对话采用“两阶段、单一事实源”：第一阶段立即启动 DeepSeek SSE（服务端持续推送）生成对方原话，并在随机 1–3 秒揭示窗口内开始透传；第二阶段只能基于这句已经展示的原话生成公开分析和下一句建议，不能改写它。若绝对 3 秒内没有获得有效首字，整轮固定使用能承接历史的本地降级结果，不会中途切换来源。八字外挂开启时只传递娱乐化沟通偏好，不向主聊天模型发送出生日期和地点。
